...
#[cfg_attr(
    feature = "alloc",
    doc = r#"```rust
# use reco::book::alekhine_defense::TWO_PAWNS_ATTACK;
assert_eq!(TWO_PAWNS_ATTACK.original_name(), "Alekhine Defense: Two Pawns Attack");
```"#
)]
pub static TWO_PAWNS_ATTACK: Variation = Variation {
    name: "Two Pawns Attack",
    parent: Some(&super::ALEKHINE_DEFENSE),
    variations: &[&LASKER_VARIATION, &MIKENAS_VARIATION, &TATE_VARIATION],
    lines: &[Line {
        parent: &TWO_PAWNS_ATTACK,
        code: Code {
            volume: Volume::B,
