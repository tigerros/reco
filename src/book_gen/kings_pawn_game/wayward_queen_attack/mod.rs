...
#[cfg_attr(
    feature = "alloc",
    doc = r#"```rust
# use reco::book::kings_pawn_game::WAYWARD_QUEEN_ATTACK;
assert_eq!(WAYWARD_QUEEN_ATTACK.original_name(), "King's Pawn Game: Wayward Queen Attack");
```"#
)]
pub static WAYWARD_QUEEN_ATTACK: Variation = Variation {
    name: "Wayward Queen Attack",
    parent: Some(&super::KINGS_PAWN_GAME),
    variations: &[&KIDDIE_COUNTERGAMBIT, &MELLON_GAMBIT],
    lines: &[Line {
        parent: &WAYWARD_QUEEN_ATTACK,
        code: Code {
            volume: Volume::C,
