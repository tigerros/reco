...
#[cfg_attr(
    feature = "alloc",
    doc = r#"```rust
# use reco::book::sicilian_defense::GRAND_PRIX_ATTACK;
assert_eq!(GRAND_PRIX_ATTACK.original_name(), "Sicilian Defense: Grand Prix Attack");
```"#
)]
pub static GRAND_PRIX_ATTACK: Variation = Variation {
    name: "Grand Prix Attack",
    parent: Some(&super::SICILIAN_DEFENSE),
    variations: &[&SCHOFMAN_VARIATION],
    lines: &[Line {
        parent: &GRAND_PRIX_ATTACK,
        code: Code {
            volume: Volume::B,
        ...
        setup: Setup {
            board: if let Ok(board) = Board::try_from_bitboards(
                ByRole {
                    pawn: Bitboard(70650237139603200),
