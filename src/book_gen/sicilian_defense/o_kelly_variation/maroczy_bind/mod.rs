...
#[cfg_attr(
    feature = "alloc",
    doc = r#"```rust
# use reco::book::sicilian_defense::o_kelly_variation::MAROCZY_BIND;
assert_eq!(MAROCZY_BIND.original_name(), "Sicilian Defense: O'Kelly Variation, Maróczy Bind");
```"#
)]
pub static MAROCZY_BIND: Variation = Variation {
    name: "Maróczy Bind",
    parent: Some(&super::O_KELLY_VARIATION),
    variations: &[&GELLER_LINE, &PAULSEN_LINE, &ROBATSCH_LINE],
    lines: &[Line {
        parent: &MAROCZY_BIND,
        code: Code {
            volume: Volume::B,
